...
class LoginActivity : AppCompatActivity() {
  ...
  private fun googleSignInClient(): GoogleSignInClient {
    ...
    account?.apply {
      ...
      val user = User(id = id, name = displayName, email = email, photoURL = photoUrl)
  ...
  private fun handleSignInResult(completedTask: Task<GoogleSignInAccount>) {
    try {
      ...
      val user = User(id = account?.id, name = account?.displayName, email = account?.email, photoURL = account?.photoUrl)
      loginViewModel.login(user)
